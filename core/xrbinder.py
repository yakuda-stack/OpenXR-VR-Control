...
Dieses Modul ist die Logik ohne Oberflaeche:
  ...
  * Bauen:     xrBinder aus dem Quellcode (git + cmake) nach
               ~/.config/openxr-vr-control/tools/xrbinder/ — im sichtbaren
...
# --------------------------------------------------------------------------- #
#  Gemeinsame Installation mit Yakuda Connect
# --------------------------------------------------------------------------- #
# OpenXR-VR-Control ist der ausgelagerte Controls-Tab von Yakuda Connect. Hat
# Yakuda Connect xrBinder schon gebaut, wird DIESE Installation benutzt statt
# einer zweiten: beide Programme teilen sich sonst ohnehin dasselbe Layer-
# Manifest, denselben IPC-Dienst und ~/.config/xrBinder — zwei Builds wuerden
# sich dort nur gegenseitig ueberschreiben (Schalter springt aus, Dienst
# zeigt auf den falschen Ordner).
YC_APP_NAME = "yakuda-connect"


def yc_config_roots():
    """Moegliche Einstellungsordner von Yakuda Connect (alt zuerst, wie dort)."""
    roots = [os.path.join(_home(), ".config", YC_APP_NAME),
             os.path.join(_xdg("XDG_CONFIG_HOME", ".config"), YC_APP_NAME)]
    return list(dict.fromkeys(roots))


def yc_config_root():
    """Einstellungsordner von Yakuda Connect oder '' (nicht installiert)."""
    return next((r for r in yc_config_roots() if os.path.isdir(r)), "")


def _built_at(root):
    out = os.path.join(root, "build", LAYER_NAME)
    return (os.path.isfile(os.path.join(out, "libxrBinder_module.so"))
            and os.access(os.path.join(out, "ipc_server"), os.X_OK))
...
def yc_tool_root():
    root = yc_config_root()
    return os.path.join(root, "tools", "xrbinder") if root else ""


def tool_root():
    """
    Ordner der benutzten xrBinder-Installation:
      1. eigener Build (~/.config/openxr-vr-control/tools/xrbinder)
      2. sonst der von Yakuda Connect, falls dort gebaut
      3. sonst eigener Ordner (dort wird gebaut)
    """
    own = own_tool_root()
    yc = yc_tool_root()
    own_ok = _built_at(own)
    yc_ok = bool(yc) and _built_at(yc)
    if own_ok and yc_ok:
        # Beide gebaut (aeltere Version dieser App): die nehmen, auf die das
        # Layer-Manifest gerade zeigt — sonst stuende der Schalter hier „aus“.
        active = _manifest_library()
        if active and os.path.realpath(active).startswith(os.path.realpath(yc) + os.sep):
            return yc
        return own
    if own_ok:
        return own
...
def _manifest_library():
    """library_path aus unserem Layer-Manifest ('' wenn keins)."""
    import json
    try:
        with open(manifest_path(), encoding="utf-8") as fh:
            return str(json.load(fh).get("api_layer", {}).get("library_path") or "")
    except (OSError, ValueError, AttributeError):
        return ""


def uses_yakuda_connect_build():
    """True, wenn die xrBinder-Installation von Yakuda Connect benutzt wird."""
    yc = yc_tool_root()
    return bool(yc) and os.path.realpath(tool_root()) == os.path.realpath(yc)
...
def state_dir():
    """
    Gemerkte Aktionen/Umbelegungen je Spiel. Ist Yakuda Connect installiert,
    wird SEIN Ordner benutzt: die daraus erzeugten app_<Spiel>.ini liegen in
    ~/.config/xrBinder und gelten fuer beide Programme — getrennte Staende
    wuerden sich beim Speichern gegenseitig ueberschreiben.
    """
    yc = yc_config_root()
    if yc:
        return os.path.join(yc, "xrbinder")
    import paths
...
def needs_rebuild():
    ...
    if have == PATCH_LEVEL:
        return False
    # Neuerer Patch (z. B. von einem aktuelleren Yakuda Connect gebaut)?
    # Dann NICHT „neu bauen“ anbieten — das waere ein Rueckschritt.
    try:
        return int(have) < int(PATCH_LEVEL)
    except ValueError:
        return True
...
def service_unit_text(port=DEFAULT_PORT):
    return (
        "# Automatisch erzeugt von openxr-vr-control\n"
        "[Unit]\n"
...
# Automatisch erzeugt von openxr-vr-control — baut xrBinder aus dem Quellcode.
