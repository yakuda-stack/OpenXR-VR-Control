...
==========================================
Je Werkzeug ein Schalter (obah). Wird er eingeschaltet:

  1. Ist das Tool installiert?  -> Schalter bleibt an, fertig.
  2. Nicht installiert          -> Rueckfrage: WIE installieren? (Cargo, yay, paru)
  3. Gewaehlte Methode          -> Installation in einem sichtbaren Terminal
                                   (CargoInstallWorker / InstallWorker).
  4. Ergebnis                   -> Erfolg: Schalter bleibt an.
                                   Fehler/Abbruch: Schalter geht wieder aus.

Aus Yakuda Connect uebernommen. Einziger Unterschied: dort lief die
Installation ueber den Tools-Tab, hier startet _start_control_install()
den Worker selbst.

Der Zustand der Schalter wird in config.json gemerkt (controls_<key>).

Auch das ist ein MIXIN — self.ui stammt aus dem Hauptfenster (core/main.py).
"""
import os
import shutil
import subprocess
...
class ControlsTabMixin:
    ...
    def _start_control_install(self, key, method):
        """
        Installation starten. Anders als in Yakuda Connect gibt es hier
        keinen Tools-Tab — der passende Worker wird direkt gestartet
        (gleiche Worker, gleiches sichtbares Terminal).
        """
        ...
        if method == "cargo":
            ...
            worker = CargoInstallWorker(tool, lang=get_language())
        elif method in ("yay", "paru") and tool.get("pkg"):
            from install_worker import InstallWorker
            worker = InstallWorker([tool["pkg"]], helper=method)
        else:
            QMessageBox.information(self, tr("controls_install_title").format(
                name=tool.get("name", key)), tr("tools_no_method"))
            self._set_control_toggle(key, False)
            return
        self._controls_pending.add(key)
        self._render_control(key, False, installing=True)
        worker.finished_signal.connect(
            lambda success, k=key: self.on_control_install_finished(k, success))
    ...
    def start_control_tool(self, key):
        ...
        for cand in (os.path.join(appimg.LOCAL_BIN, cmd),
                     ...
                     # von Yakuda Connect per Cargo gebaut
                     os.path.join(home, ".config", "yakuda-connect", "tools", "cargo", key,
                                  "bin", cmd)):
