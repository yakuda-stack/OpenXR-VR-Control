...
Ablauf fuer obah (key="obah", crate="obah", start_cmd="obah"):
  ...
  1. Ordner anlegen:
        ~/.config/openxr-vr-control/tools/cargo/obah/
...
def tool_root(tool):
    """~/.config/openxr-vr-control/tools/cargo/<key>"""
...
def latest_revision(tool, timeout=8):
    ...
    if url.startswith(prefix):
        ...
        req = urllib.request.Request(
            ...
            headers={"User-Agent": "openxr-vr-control", "Accept": "application/vnd.github+json"})
        try:
            with urllib.request.urlopen(req, timeout=timeout) as r:
...
# Automatisch erzeugt von openxr-vr-control — Cargo-Installation von {crate}.
