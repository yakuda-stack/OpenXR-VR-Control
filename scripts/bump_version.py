...
den alten Tag; vergisst man die README, zeigt das Badge eine falsche Version.
...
Gepflegt werden:
  ...
  2. packaging/aur/PKGBUILD    -> pkgver=1.1.6 und pkgrel=1
  3. packaging/aur/.SRCINFO    -> pkgver, pkgrel und source-Zeile
  4. README.md                 -> Versions-Badge (shields.io)
  5. CHANGELOG.md / HIGHLIGHTS.md -> nur die Ueberschrift
                                  "### 🚀 v1.1.6 — JJJJ-MM-TT" ganz oben
                                  (den Text schreibst du selbst)

--check prueft alle. Bei CHANGELOG/HIGHLIGHTS muss die aktuelle
...
(Aus Yakuda Connect uebernommen — ohne den APP_VERSION-Anker in core/main.py,
...
def check(expect=None):
    ...
    for label, value in (("PKGBUILD", v_pkg),):
...
def bump(new_version, date=None):
    ...
    print(f"""
Naechste Schritte (oder einfach release_launcher.py benutzen):
  ...
  4. AUR: cd ~/aur/openxr-vr-control -> pkgver/pkgrel angleichen ->
