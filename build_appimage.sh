#!/usr/bin/env bash
# openxr-vr-control — AppImage Builder (EINE Datei, laeuft mit fuse2 UND fuse3)
# ============================================================================
# Verwendung:
...
#   ./openxr-vr-control-*.AppImage --appimage-extract-and-run
...
UPDATE_INFO="${UPDATE_INFO:-gh-releases-zsync|yakuda-stack|OpenXR-VR-Control|latest|${APP}-*-${ARCH}.AppImage.zsync}"
RUNTIME_URL="https://github.com/AppImage/type2-runtime/releases/download/continuous/runtime-${ARCH}"
CACHE_DIR="${XDG_CACHE_HOME:-$HOME/.cache}/openxr-vr-control/build"
...
echo "=== OpenXR/VR-Controls AppImage Builder ==="
...
mkdir -p "$BUILD_DIR/usr/bin" \
         "$BUILD_DIR/usr/lib/openxr-vr-control" \
...
cp -r assets config core ui locales "$BUILD_DIR/usr/lib/openxr-vr-control/"
cp starter.py "$BUILD_DIR/usr/lib/openxr-vr-control/"
# Werden in der App angezeigt (Einstellungen -> Changelog / Highlights)
cp CHANGELOG.md HIGHLIGHTS.md "$BUILD_DIR/usr/lib/openxr-vr-control/"
# Bytecode und Entwicklerreste gehoeren nicht in die Auslieferung
find "$BUILD_DIR/usr/lib/openxr-vr-control" -type d -name '__pycache__' -prune -exec rm -rf {} +
find "$BUILD_DIR/usr/lib/openxr-vr-control" -type f -name '*.py[co]' -delete
# Screenshots aus assets/ wuerden die AppImage nur aufblaehen
rm -f "$BUILD_DIR/usr/lib/openxr-vr-control/assets/"{screenshot_*,openvrcontrol,openxrcontrol}.png

cat > "$BUILD_DIR/usr/bin/openxr-vr-control" << 'WRAPPER'
#!/bin/bash
cd "$(dirname "$(readlink -f "$0")")/../lib/openxr-vr-control" || exit 1
exec python3 starter.py "$@"
WRAPPER
chmod +x "$BUILD_DIR/usr/bin/openxr-vr-control"

echo "      Setze Icon und Desktop-Eintrag..."
cp assets/icon.svg "$BUILD_DIR/usr/share/icons/hicolor/scalable/apps/openxr-vr-control.svg"
cp assets/icon.png "$BUILD_DIR/usr/share/icons/hicolor/512x512/apps/openxr-vr-control.png"
cp assets/icon.svg "$BUILD_DIR/openxr-vr-control.svg"

# Desktop-Eintrag = die .download-Variante (Exec/Icon ohne Pfad) + Version
cp openxr-vr-control.desktop.download "$BUILD_DIR/usr/share/applications/openxr-vr-control.desktop"
echo "X-AppImage-Version=$VERSION" >> "$BUILD_DIR/usr/share/applications/openxr-vr-control.desktop"
cp "$BUILD_DIR/usr/share/applications/openxr-vr-control.desktop" "$BUILD_DIR/openxr-vr-control.desktop"
...
if ! command -v python3 >/dev/null 2>&1; then
    MSG="OpenXR/VR-Controls braucht Python 3 auf dem System.
Bitte installieren:
...
exec "$HERE/usr/bin/openxr-vr-control" "$@"
APPRUN
