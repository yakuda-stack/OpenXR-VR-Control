#!/bin/bash
# OpenXR/VR-Controls — Installer & Updater (Arch, Fedora, Debian/Ubuntu, openSUSE)
#
...
#
# Re-Run = Update: holt den aktuellen main-Stand und ersetzt /opt/openxr-vr-control.
# Deine Einstellungen unter ~/.config/openxr-vr-control bleiben unangetastet.
set -e

INSTALL_DIR="/opt/openxr-vr-control"
VENV_DIR="/opt/openxr-vr-control-venv"   # nur Fallback, wenn es kein Distro-PySide6 gibt
DESKTOP_FILE="/usr/share/applications/openxr-vr-control.desktop"
ICON_DIR="/usr/share/icons/hicolor/256x256/apps"
BIN_LINK="/usr/local/bin/openxr-vr-control"
REPO="https://github.com/yakuda-stack/OpenXR-VR-Control"
BRANCH="main"
...
sudo install -Dm644 "$INSTALL_DIR/assets/icon.svg" "$SVG_ICON_DIR/openxr-vr-control.svg"
sudo install -Dm644 "$INSTALL_DIR/assets/icon.png" "/usr/share/icons/hicolor/512x512/apps/openxr-vr-control.png"
sudo gtk-update-icon-cache -f /usr/share/icons/hicolor 2>/dev/null || true
# .download-Variante (Exec/Icon ohne Pfad), Exec auf den Startbefehl umbiegen
sed "s#^Exec=.*#Exec=$BIN_LINK#" "$INSTALL_DIR/openxr-vr-control.desktop.download" \
...
echo ""
echo "[OK] OpenXR/VR-Controls ist auf Version $NEW_VER ($MODE abgeschlossen)."
echo ""
echo "Starten:"
echo "  - Im Anwendungsmenue: nach 'OpenXR/VR-Controls' suchen"
echo "  - Im Terminal: openxr-vr-control"
