...
===================================================================
Steht oben im Controls-Tab neben obah und sieht genauso aus:
...
class XrBinderCard(QFrame):
    ...
    def render(self):
        ...
        if xb.is_built() and xb.uses_yakuda_connect_build():
            status += "  · " + tr("xrb_from_yc")
            self.lbl_status.setToolTip(tr("xrb_from_yc_tip").format(path=xb.tool_root()))
        else:
